 ...
 * By Junnan Li
'''
from models.gemformer import BertConfig, BertLMHeadModel
...
class BLIP_Pretrain(nn.Module):
    def __init__(self,                 
        ...
        self.visual_encoder, vision_width = create_clip_vit(config['vision_model'],config['image_size'])
        ...
        self.text_model = BertLMHeadModel.from_pretrained(config['text_model'],config=encoder_config)   
        ...
        self.query_tokens = nn.Parameter(torch.zeros(1, config['num_query_token'], encoder_config.hidden_size))       
        self.query_tokens.data.normal_(mean=0.0, std=encoder_config.initializer_range)
        
        self.vision_proj = nn.Linear(self.text_model.config.hidden_size, config['embed_dim'])
        self.text_proj = nn.Linear(self.text_model.config.hidden_size, config['embed_dim'])
    ...
    def compute_itc(self, image, text):
        ...
        query_output = self.text_model.bert(query_embeds = query_tokens,
        ...
        text_tokens = self.tokenizer(text, padding='longest', truncation=True, max_length=self.max_text_length, return_tensors="pt").to(image.device)  
        text_output = self.text_model.bert(text_tokens.input_ids, 
    ...
    @autocast()
    def compute_itm(self, image_embeds, text, return_prob = False):
        image_atts = torch.ones(image_embeds.size()[:-1],dtype=torch.long).to(image_embeds.device)     
        
        query_tokens = self.query_tokens.expand(image_embeds.shape[0], -1, -1)
        query_atts = torch.ones(query_tokens.size()[:-1],dtype=torch.long).to(image_embeds.device)      
        
        text_tokens = self.tokenizer(text, padding='longest', truncation=True, max_length=self.max_text_length, return_tensors="pt").to(image_embeds.device) 
        attention_mask = torch.cat([query_atts, text_tokens.attention_mask],dim=1)
        
        output_itm = self.text_model.bert(text_tokens.input_ids,
        ...
        if return_prob:
            vl_output = vl_output.mean(dim=1)
            prob = F.softmax(vl_output,dim=1)
            return prob[:,1]
...
#             vl_embeddings.append(output_itm.last_hidden_state[i,:input_len[i],:].mean(0))
#         vl_embeddings = torch.stack(vl_embeddings,dim=0)    
#         vl_output = self.itm_head(vl_embeddings)         
#         vl_output = vl_output[:,1]
    ...
    def forward(self, image, text):
        ...
        text_tokens = self.tokenizer(text, padding='max_length', truncation=True, max_length=self.max_text_length, return_tensors="pt").to(image.device)  
        text_output = self.text_model.bert(text_tokens.input_ids, 
                                             attention_mask = text_tokens.attention_mask,
                                             return_dict = True)            
        text_feat = F.normalize(self.text_proj(text_output.last_hidden_state[:,0,:]),dim=-1) 
        ...
        image_feats_all = all_gather(image_feats) # [batch_size*num_gpu, num_query_tokens, embed_dim]
        ...
        smooth = 0.1
        loss_itc = (F.cross_entropy(sim_i2t,targets,label_smoothing=smooth)+
                F.cross_entropy(sim_t2i,targets,label_smoothing=smooth))/2
...
#             text_atts_neg.append(text_tokens.attention_mask[neg_idx])
        ...
        output_itm = self.text_model.bert(text_ids_all,
...
#         input_len = attention_mask_all.sum(1)
#         vl_embeddings = []
#         for i in range(attention_mask_all.size(0)):
#             vl_embeddings.append(output_itm.last_hidden_state[i,:input_len[i],:].mean(0))
#         vl_embeddings = torch.stack(vl_embeddings,dim=0)    
#         logits = self.itm_head(vl_embeddings)   
        ...
        attention_mask = torch.cat([query_atts,text_tokens.attention_mask],dim=1)
        lm_output = self.text_model(decoder_input_ids, 
                                    attention_mask = attention_mask, 
    ...
    @autocast()
    def generate(self, image, prompt='', sample=False, num_beams=3, max_length=25, min_length=5, top_p=0.9):
        image_embeds = self.visual_encoder(image)
        image_embeds = self.ln_vision(image_embeds)
        ...
        if sample:
            #nucleus sampling
            outputs = self.text_model.generate(input_ids=input_ids,
                                                 query_embeds=query_tokens,
