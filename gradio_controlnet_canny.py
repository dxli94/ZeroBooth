...
def create_model():
    ...
    model = ZeroBooth(config=config.model, control_type="canny")
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            with gr.Row():
                ...
                canny_image = gr.Image(type="pil", interactive=True, label="canny image")
            
            with gr.Row():
                canny_low_threshold = gr.Slider(
                    label="canny low threshold", minimum=0, maximum=255, value=100, step=5
                )
                canny_high_threshold = gr.Slider(
                    label="canny high threshold", minimum=0, maximum=255, value=200, step=5
                )

            cond_image_input.change(
                generate_canny,
                inputs=[cond_image_input, canny_low_threshold, canny_high_threshold],
                outputs=[canny_image],
            )
...
if __name__ == "__main__":
    ...
    iface.launch(enable_queue=True, server_port=7863)
