...
def main(args):
    ...
    # )
    model_captioner = load_model(
        name="blip2_opt", model_type="pretrain_opt6.7b", device=device, is_eval=True
    )
        ...
        for batch_id, (image_blip, image_clip, labels, image_ids) in enumerate(
            ...
            # for i in range(len(interleave_labels)):
                # if i % 2 == 0:
                    ...
                    # prompts.append("a {} photo, ".format(interleave_labels[i]).lower())
                #     prompts.append("a {} photo, ".format(interleave_labels[i]).lower())
                # else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_cap", default=48, type=int)
    ...
    parser.add_argument(
        ...
        default="/export/home/workspace/dreambooth/diffusers/data/openimage-annotations/{}_opt6b7".format(split)
    )
